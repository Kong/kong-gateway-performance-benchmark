...
usage() {
  cat <<'EOF'
Usage:
  ./run_upper_token_campaign.sh [repeats] [results-dir]

Defaults:
  repeats      3
  results-dir  ./results/upper-token-campaign-<timestamp>

Default bracket specs:
  9000:14000:20000
  10000:16000:24000
  11000:18000:28000

Each bracket entry is:
  rate_rps:preAllocatedVUs:maxVUs
EOF
  exit 1
}
...
REPEATS=${1:-3}
RESULTS_DIR=${2:-"./results/upper-token-campaign-$(date -u +"%Y%m%dT%H%M%SZ")"}

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
mkdir -p "$RESULTS_DIR"

SCENARIO="token-chat-openai"
FIXTURE=${UPPER_TOKEN_FIXTURE:-short}
DURATION=${UPPER_TOKEN_DURATION:-6m}
BRACKET_SPECS=(${UPPER_TOKEN_BRACKET_SPECS:-9000:14000:20000 10000:16000:24000 11000:18000:28000})
...
write_summary_line() {
  ...
  local http_p95 http_p99 http_failed checks_pass
  local insufficient_vus_warning_count
  ...
  insufficient_vus_warning_count=$(grep -c 'Insufficient VUs' "$runner_log" || true)
...
run_one() {
  ...
  local run_name="${SCENARIO}_rps${load_value}_r${repeat_index}"
  ...
  echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] starting $run_name preAllocatedVUs=$pre_allocated_vus maxVUs=$max_vus"
  ...
  K6_AI_PRE_ALLOCATED_VUS_OVERRIDE=$pre_allocated_vus K6_AI_MAX_VUS_OVERRIDE=$max_vus \
  ...
  write_summary_line "$summary_file" "$load_value" "$pre_allocated_vus" "$max_vus" "$repeat_index" "$run_dir" "$pod_phase"
...
metric_fields = [
    ...
    'k6_runner_cpu_peak_of_limit_percent',
    'k6_runner_memory_peak_of_limit_percent',
]
...
for load_value, group in sorted(groups.items(), key=lambda item: float(item[0])):
    output = {
        ...
        'runs': len(group),
        'pre_allocated_vus': group[0]['pre_allocated_vus'],
        'max_vus': group[0]['max_vus'],
        ...
        'warning_runs': sum(1 for row in group if to_float(row['insufficient_vus_warning_count']) > 0),
    }
...
fieldnames = ['load_value', 'runs', 'pre_allocated_vus', 'max_vus', 'succeeded_runs', 'failed_runs', 'warning_runs']
for field in metric_fields:
    fieldnames.extend([
        f'{field}_median',
        f'{field}_min',
...
findings = ['# Upper token bracket findings', '']
if aggregate_rows:
    findings.append(f'- Total completed runs: {sum(int(row["runs"]) for row in aggregate_rows)}')
    findings.append('')

for row in aggregate_rows:
    findings.append(f'## {row["load_value"]} RPS')
    findings.append('')
    findings.append(f'- VU settings: preAllocatedVUs={row["pre_allocated_vus"]}, maxVUs={row["max_vus"]}')
    findings.append(f'- Pod phases: succeeded={row["succeeded_runs"]}, failed={row["failed_runs"]}')
    findings.append(f'- `Insufficient VUs` warnings: {row["warning_runs"]}/{row["runs"]} runs')
    findings.append(f'- Achieved req/s median: **{row["http_requests_per_second_median"]:.2f}** (range {row["http_requests_per_second_min"]:.2f} -> {row["http_requests_per_second_max"]:.2f})')
    findings.append(f'- Dropped iterations median: **{row["dropped_iterations_total_median"]:.0f}**')
    ...
    findings.append(f'- Upstream node peak util median: cpu **{row["upstream_node_cpu_peak_of_allocatable_percent_median"]:.2f}%**, mem **{row["upstream_node_memory_peak_of_allocatable_percent_median"]:.2f}%**')
...
focus_row = None
for row in aggregate_rows:
    if int(row['succeeded_runs']) != int(row['runs']) or int(row['warning_runs']) > 0:
        focus_row = row
        break
if focus_row is None and aggregate_rows:
    focus_row = aggregate_rows[-1]

if focus_row:
    findings.append('## Bottleneck hint')
    findings.append('')
    domain_scores = {
        'kong': max(
            focus_row['kong_node_cpu_peak_of_allocatable_percent_median'],
            focus_row['kong_node_memory_peak_of_allocatable_percent_median'],
        ),
        'upstream': max(
            focus_row['upstream_node_cpu_peak_of_allocatable_percent_median'],
            focus_row['upstream_node_memory_peak_of_allocatable_percent_median'],
        ),
        'k6/loadgen': max(
            focus_row['k6_node_cpu_peak_of_allocatable_percent_median'],
            focus_row['k6_node_memory_peak_of_allocatable_percent_median'],
            focus_row['k6_runner_cpu_peak_of_limit_percent_median'],
            focus_row['k6_runner_memory_peak_of_limit_percent_median'],
        ),
    }
    candidate, score = max(domain_scores.items(), key=lambda item: item[1])
    findings.append(f'- Focus point: **{focus_row["load_value"]} RPS**')
    findings.append(f'- Strongest recorded saturation signal: **{candidate}** at **{score:.2f}%** of recorded capacity')
    findings.append('- Use this as triage guidance with `summary.csv`, `aggregate.csv`, and raw `infra_timeseries.csv`, not as a standalone proof.')
    findings.append('')
...
for spec in "${BRACKET_SPECS[@]}"; do
  IFS=: read -r load_value pre_allocated_vus max_vus <<<"$spec"
  for repeat_index in $(seq 1 "$REPEATS"); do
    run_one "$load_value" "$pre_allocated_vus" "$max_vus" "$repeat_index" "$SUMMARY_FILE"
  done
done

aggregate_results "$SUMMARY_FILE" "$AGGREGATE_FILE" "$FINDINGS_FILE"

echo "Upper token campaign completed."

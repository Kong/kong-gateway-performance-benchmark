...
export default function () {
  const response = http.post(
    __ENV.K6_AI_CHAT_URL || 'https://kong-kong-proxy.kong.svc.cluster.local/bench/token/chat/openai',
    JSON.stringify(buildOpenAiChatRequest(fixture, false)),
    {
      timeout: __ENV.K6_AI_TIMEOUT || '60s',
      headers: buildTunedHeaders(fixture),
      tags: {
        route: 'bench-token-chat-openai',
        mode: 'non-streaming',
      },
    },
  )
...
export function handleSummary(data) {
  return renderAiSummary(
    `token chat openai (${__ENV.K6_AI_FIXTURE || 'short'})`,
